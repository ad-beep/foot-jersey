    ...
    <Reveal>
      <div className={`mb-8 ${isHe ? 'text-right' : ''}`}>
        <p className="section-kicker mb-3">
          {isHe ? 'הכי חם עכשיו' : "This Week's Drop"}
        </p>
        <div className={`flex items-end justify-between gap-4 ${isHe ? 'flex-row-reverse' : ''}`}>
          ...
          <Link
            ...
            className="shrink-0 font-mono text-xs uppercase tracking-widest pb-1 border-b transition-colors duration-200"
    ...
    >
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 w-full">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4" style={{ scrollSnapType: 'x mandatory' }}>
            {hotJerseys.slice(0, 5).map((jersey, i) => (
              <div key={jersey.id} className="shrink-0" style={{ width: cardWidth, scrollSnapAlign: 'center' }}>
                <ProductCard jersey={jersey} priority={i < 3} imageSizes="(max-width: 768px) 280px, 320px" imageQuality={50} />
              </div>
            ))}
          </div>
        )}
                ...
                >
                  {isCenter && (
                    <div
                      ...
                      style={{ boxShadow: '0 0 32px rgba(200,162,75,0.18)' }}
                    />
                  )}
                  <ProductCard jersey={jersey} priority={isCenter} imageSizes="(max-width: 768px) 280px, 320px" imageQuality={50} />
                </motion.div>
              );
            })}
          </div>
        ...
        <div className="mt-6 max-w-sm mx-auto">
          <div className="h-px rounded-full overflow-hidden" style={{ backgroundColor: 'var(--border)' }}>
            <motion.div
          ...
          <p className="font-mono text-[10px] text-center mt-2 uppercase tracking-widest" style={{ color: 'var(--muted)' }}>
