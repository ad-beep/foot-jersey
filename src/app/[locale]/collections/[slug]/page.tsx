      ...
      <div className="min-h-screen" style={{ backgroundColor: 'var(--ink)' }}>
        ...
        >
          ...
          <div className={`relative max-w-[900px] mx-auto px-4 md:px-6 ${isHe ? 'text-right' : ''}`}>
            ...
            <Link
              href={discoverHref}
              className="inline-flex items-center gap-3 px-7 py-3.5 rounded-xl font-bold text-sm tracking-wide transition-all duration-200 active:scale-95"
          ...
          <div className={`grid md:grid-cols-[1fr_2fr] gap-12 ${isHe ? 'md:grid-cols-[2fr_1fr]' : ''}`}>
            ...
            <div className={`${isHe ? 'text-right' : ''}`}>
              ...
              <div
                ...
                style={{ fontSize: 'clamp(4rem, 12vw, 9rem)', color: `${col.accent}18`, lineHeight: 1, letterSpacing: '-0.05em', userSelect: 'none' }}
                aria-hidden="true"
              >
            ...
            <div className={`${isHe ? 'text-right' : ''}`}>
              ...
              <Link
                href={discoverHref}
                className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-widest pb-0.5 border-b transition-colors duration-200"
                style={{ color: 'var(--muted)', borderColor: 'var(--border)' }}
                onMouseEnter={(e) => {
                  (e.currentTarget as HTMLElement).style.color = col.accent;
                  (e.currentTarget as HTMLElement).style.borderColor = col.accent;
                }}
                onMouseLeave={(e) => {
              ...
              {COLLECTIONS.filter((other) => other.slug !== col.slug).map((other) => {
                ...
                return (
                  <Link
                    ...
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLElement).style.borderColor = other.accent;
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)';
                    }}
